		...
		while (delta > 0.2) {
			...
			for (SentencePair p : trainingPairs) {
				...
				for (int i = 0; i < sourceWords.size(); i++) {
					...
					for (int j = 0; j < targetWords.size(); j++) {
						key = i + "-" + targetWords.size() + "-" + sourceWords.size();
						sum += transProb.getCount(targetWords.get(j), sourceWords.get(i))
								* distortion.getCount(key, j);
					}

					for (int j = 0; j < targetWords.size(); j++) {
						key = i + "-" + targetWords.size() + "-" + sourceWords.size();
						prob = transProb.getCount(targetWords.get(j), sourceWords.get(i))
								* distortion.getCount(key, j);
						countAlignment.incrementCount(targetWords.get(j),
								sourceWords.get(i), prob / sum);
			...
			for (String tWord : transProb.keySet()) {
				...
				for (String sWord : sCounter.keySet()) {
					transProb.setCount(tWord, sWord,
							countAlignment.getCount(tWord, sWord) / sCountTotal);
				}
			}
